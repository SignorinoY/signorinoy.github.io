---
title: 时间序列分析学习笔记：描述性时序分析
tags: [时间序列分析, 数据可视化, R]
date: 2018-10-04 21:25:47
permalink: time-series-analysis-descriptive-analysis
---

> 早期的时序分析通常都是通过直观的数据比较或绘图观测,寻找序列中蕴涵的发展规律,这种分析方法就称为描述性时序分析.
> 操作简单、直观有效是描述性时间序列分析方法的突出特点.它通常也是人们进行统计时序分析的第一步,通过图示的方法可以直观地反映出序列的特征.
...
尝试使用`read.table`将数据读入R,并存为`temp_data`.

```r
temp_data <- read.table("../data/daily-minimum-temperatures-in-melbourne.csv", header = TRUE, sep = ",")
```
...
R 语言无法自动将读取的数据转化为时间序列格式,所以利用R语言画时间序列图的一个关键步骤就是将读取的数据转变为时间序列格式.

```r
temp_data$Date <- as.Date(temp_data$Date, format='%Y/%m/%d')
```

查看修改后的`temp_data`的内部结构,发现修改成功.
...
'data.frame':    3650 obs. of  2 variables:
 ...
 $ MinTemp: num  20.7 17.9 18.8 14.6 15.8 15.8 15.8 17.4 21.8 20 ...
```
...
> 时间序列的首选也可能是最流行的可视化方法是线图,即时间显示在x轴上,而观察值则沿着y轴的.
...
![时间序列线图](https://xn--i0v668g.com/uploads/images/time-series-analysis-descriptive-analysis-1.png)
...
![时间序列直方图](https://xn--i0v668g.com/uploads/images/time-series-analysis-descriptive-analysis-2.png)
...
![时间序列区间箱线图](https://xn--i0v668g.com/uploads/images/time-series-analysis-descriptive-analysis-3.png)